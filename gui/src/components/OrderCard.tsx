...
const OrderCard: React.FC<OrderCardProps> = ({
  ...
  const deliveryAddress = purchaseDetails
        ...
        try {
          ...
          formProps.setIsWorking(
            handleErrorForRendering(
              error,
              "Accepting delivery order and changing status to Picked Up",
            ),
          );
        }
      }
      ...
      if (formProps.dMarketApi) {
        try {
          ...
          formProps.setIsWorking(
            handleErrorForRendering(
              error,
              "Confirming delivery order was accepted and is In Transit",
            ),
          );
        }
      }
    ...
    async (offerId: string, itemName: string) => {
      if (formProps.dMarketApi) {
        try {
          ...
          formProps.setIsWorking(
            handleErrorForRendering(error, "Setting item as delivered"),
          );
        }
      }
    },
      ...
      if (formProps.dMarketApi) {
        try {
          ...
          formProps.setIsWorking(
            handleErrorForRendering(
              error,
              "Confirming item was delivered and releasing payment",
            ),
          );
        }
      }
    ...
    async (
      ...
      if (formProps.dMarketApi) {
        try {
          ...
          formProps.setIsWorking(
            handleErrorForRendering(error, `Rating ${roleToRate}`),
          );
        }
      }
    },
  ...
  const renderRating = () => {
    ...
    return (
      <div className="mt-4 pt-4 border-t border-slate-700">
        <h4 className="text-sm font-bold mb-2 text-brand-text-primary">
          Rate Participants
        </h4>
  ...
  return (
    <div className="bg-brand-surface rounded-xl shadow-lg overflow-hidden flex flex-col p-6 transition-all duration-300 hover:shadow-2xl hover:shadow-cyan-500/20 hover:-translate-y-1 border border-transparent hover:border-brand-primary/50">
      ...
      <div className="text-sm text-brand-text-secondary space-y-2 mb-4 flex-grow">
        ...
        <p>
          <span className="font-semibold text-brand-text-primary">Seller:</span>{" "}
          {seller.name}
        </p>
        <p>
          <span className="font-semibold text-brand-text-primary">Buyer:</span>{" "}
          {buyer?.name}
        </p>
        <p>
          <span className="font-semibold text-brand-text-primary">
