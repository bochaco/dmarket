...
import { UserRole, User, Offer, OfferStatus, DisputeResolutionType, DisputeReasonType, CarrierBid } from '../types';
import Header from './Header';
import WorkInProgressModal, { WorkInProgressInfo, handleErrorForRendering } from './WorkInProgressModal';
...
import { type DMarketDerivedState, type DeployedDMarketAPI } from '../../../api/src/index';
...
export const DMarket: React.FC<Readonly<DMarketProps>> = ({ dMarketDeployment$ }) => {
  const dMarketApiProvider = useDeployedDMarketContext();
  const [dMarketDeployment, setDMarketDeployment] = useState<DMarketDeployment>();
  ...
  useEffect(() => {
    if (dMarketState) {
      let updatedUsers = carriersAsUsers(dMarketState).concat(sellersAsUsers(dMarketState));
  ...
  const handleSetupComplete = useCallback(
    async (data: SetupData) => {
      if (dMarketApiProvider) {
        const accountPassword: Uint8Array = Uint8Array.from(data.password);
  ...
  const allVisibleOrders = offers.filter((offer) => {
    switch (offer.status) {
      case OfferStatus.Available || OfferStatus.Completed:
        return false;
        break;
      default:
        return true;
  ...
  const carrierPendingOrders =
    currentRole === UserRole.Carrier
      ? allVisibleOrders.filter((o) => o.status === OfferStatus.AwaitingCarrierAcceptance)
      : [];
  ...
  return (
    <div className="bg-brand-background min-h-screen text-brand-text-primary font-sans">
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-8 xl:col-span-9">
            {currentRole === UserRole.Seller && <CreateOfferForm dMarketApi={dMarketApi} setIsWorking={setIsWorking} />}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mb-12">
              ...
              {offersAvailable.length === 0 && (
                <p className="text-brand-text-secondary md:col-span-2 xl:col-span-3">No offers available.</p>
              )}
            </div>

            {currentRole === UserRole.Carrier ? (
              <>
                <h2 className="text-2xl font-bold mb-6">Awaiting Your Acceptance</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                  ...
                  {carrierActiveOrders.length === 0 && (
                    <p className="text-brand-text-secondary md:col-span-2 xl:col-span-3">No active deliveries.</p>
                  )}
                </div>
              </>
            ) : (
              <>
                <h2 className="text-2xl font-bold mb-6">
                  {currentRole === UserRole.Buyer ? 'Your Purchases' : 'Your Sales'}
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                  ...
                  {allVisibleOrders.length === 0 && (
                    <p className="text-brand-text-secondary md:col-span-2 xl:col-span-3">No offers to display.</p>
                  )}
                </div>
              </>
            )}
      ...
      {isWorking && <WorkInProgressModal isWorking={isWorking} setIsWorking={setIsWorking} />}
      ...
      {biddingOffer && (
        <BidModal offer={biddingOffer} onClose={() => setBiddingOffer(null)} formProps={{ dMarketApi, setIsWorking }} />
      )}
...
const deserializeMetadataJson = (string: string): { name: string; imageUrl: string; description: string } => {
  try {
    // Attempt to parse the JSON string
    const parsedData = JSON.parse(string);
    const url = parseUrl(parsedData.imageUrl);
    return { name: parsedData.name, imageUrl: url, description: `${parsedData.description.slice(0, 200)}...` };
  } catch (error) {
...
const updatedOffers = (dMarketState: DMarketDerivedState, updatedUsers: User[]): Offer[] => {
  let updatedOffers = [];
  for (const [id, offer] of dMarketState.offers) {
    ...
    if (sellerUser) {
      ...
      if (offer.purchaseDetails.is_some) {
        const carrier = updatedUsers.find((u) => u.id === toHex(offer.purchaseDetails.value.selectedCarrierId));
        if (carrier) {
          ...
          for (const rating of offer.sellerRatings) {
            sellerUser.ratings = calculateRatingAverage(sellerUser.ratings, rating);
          }
      ...
      const o: Offer = {
        ...
        eta: offer.deliveryEta > 0 ? new Date(parseInt(offer.deliveryEta.toString())) : null,
        name: meta.name,
...
const calculateRatingAverage = (
  ...
  let average = (BigInt(current.average * current.count) + rating) / BigInt(current.count + 1n);
