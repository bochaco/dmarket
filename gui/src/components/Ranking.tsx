...
const Ranking: React.FC<RankingProps> = ({ users }) => {
    ...
    return users
      ...
      .sort((a, b) => {
        if (b.ratings.average !== a.ratings.average) {
          return Number(b.ratings.average) - Number(a.ratings.average);
        }
        return Number(b.ratings.count) - Number(a.ratings.count); // Tie-breaker
      })
      .slice(0, 5);
  };
  ...
  const RankingList = ({ title, data }: { title: string; data: User[] }) => (
    <div className="mb-6">
      <h3 className="text-lg font-bold text-brand-text-primary mb-3">
        {title}
      </h3>
      <ul className="space-y-2">
        {data.length > 0 ? (
          data.map((user, index) => (
            <li
              key={user.id}
              className="flex justify-between items-center bg-brand-background p-2 rounded-md"
            >
              ...
              <div className="flex items-center gap-2">
                <RankingInput
                  readOnly
                  currentRating={user.ratings.average}
                  size="sm"
                />
                <span className="text-xs font-semibold text-brand-primary w-8 text-right">
                  ({user.ratings.count})
                </span>
              </div>
            </li>
  ...
  return (
    <div className="bg-brand-surface p-6 rounded-xl shadow-2xl shadow-slate-900/50 sticky top-24 border border-slate-700">
      <h2 className="text-2xl font-bold mb-6 text-center text-brand-text-primary">
        Top Performers
      </h2>
